...
<body>
    ...
    <main>
        ...
        <!----------搜尋框--------->
		   ...
		   <!--<div class="sbox">-->
			<div class="sbox">
            <input type="search"  placeholder="Search..." name="id" class="searchbad"/>
			</div>
        ...
        <div class="introduction">
            <ul class="list">
                <li class="product_pt">
					<%
					sql = "SELECT `id`,`name`,`img`,`content`,`price`,`figure`FROM `product` WHERE `id`="+id ;
					...
					{			
						...
						out.println("★價錢:"+tmp.getString(5)+"<br /></p>");
						out.println(" <div class='buy_information'>");
						
						
						out.println("<form action='addshoppingcar.jsp' class='buy_quantitybox'><p class='buy_quantity'>訂購數量:</p><input min='1' type='number' name='quantity' value='1' class='car_button'>");
					session.setAttribute("id",id);
					session.setAttribute("proname",tmp.getString(2));
					}
					//con.close();
					%>
                    <!----直接加入購物車按鈕---->
                    <input type="submit" value="加入購物車"                class="gotoshoppingcar">
					</form>	
					<%out.println("</div>");%>
           ...
           sql="SELECT * FROM `board` WHERE `proid`='"+id+"' ORDER BY `boid` DESC LIMIT ";
           sql+=start_record+",5";
